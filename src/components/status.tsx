...
import { GAME_PHASES, OUTCOME } from "@/core/game/constants.ts";
...
export function Status() {
  ...
  return (
    TIMER_PHASES.includes(phase) && (
      <VStack gap="2rem">
        <ProgressCircleRoot value={turnPercent} size="xl" colorPalette="purple">
          <ProgressCircleRing css={{ "--thickness": "0.25rem" }} cap="round" />
          <ProgressCircleValueText fontSize="lg">
            {phase === GAME_PHASES.PLAYERS_TURN && Math.round(time).toString()}
        ...
        <AnimatePresence initial={false}>
          {phase === GAME_PHASES.GAME_RESULTS && (
            ...
            >
              ...
              {/* TODO: online play only */}
              {/*<Button variant="surface" w="full">*/}
              {/*  Find new opponent*/}
              {/*</Button>*/}
              <Button variant="ghost" w="full" onClick={returnToMenu}>
                Main menu
              </Button>
            </MotionStack>
          )}
